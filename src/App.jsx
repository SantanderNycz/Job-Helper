...
export default function App() {
  ...
  const [links, setLinks] = useState([
    {
      ...
      value: "",
      locked: false,
      placeholder: translations.en.github,
    },
    {
      type: "linkedin",
      value: "",
      locked: false,
      placeholder: translations.en.linkedin,
    },
  ]);
  ...
  useEffect(() => {
    setLinks((prev) =>
      prev.map((l) => {
        if (l.type === "github")
          return { ...l, placeholder: translations[language].github };
        if (l.type === "linkedin")
          return { ...l, placeholder: translations[language].linkedin };
        return l;
      })
    );
  }, [language]);
  ...
  useEffect(() => {
    if (language === "en" && skills.pt && !skills.en)
      setSkills((prev) => ({ ...prev, en: skills.pt }));
    if (language === "pt" && skills.en && !skills.pt)
      setSkills((prev) => ({ ...prev, pt: skills.en }));
    if (language === "en" && interest.pt && !interest.en)
      setInterest((prev) => ({ ...prev, en: interest.pt }));
    if (language === "pt" && interest.en && !interest.pt)
      setInterest((prev) => ({ ...prev, pt: interest.en }));
  }, [language]);

  const handleSkillsChange = (v) =>
    setSkills((prev) => ({ ...prev, [language]: v }));
  const handleInterestChange = (v) =>
    setInterest((prev) => ({ ...prev, [language]: v }));

  const addLink = () =>
    setLinks((prev) => [
      ...prev,
      { type: "custom", value: "", locked: false, placeholder: "Custom link" },
    ]);
  ...
  const generateLetter = () => {
    ...
    const interestText =
      includeInterest && interest[language]
        ? language === "en"
      ...
      <div className="w-full max-w-3xl bg-white shadow rounded-lg p-6 space-y-4">
        ...
        <label className="flex items-center gap-2 text-sm">
          <input
            ...
            onChange={() => setIncludeInterest((v) => !v)}
          />
        ...
        <div className="flex items-center gap-4">
          <button onClick={addLink} className="text-[#0A66C2] underline">
            {t.addLink}
          </button>
        ...
        <div>
          ...
          <div className="border p-4 rounded min-h-[120px] whitespace-pre-line bg-gray-50">
            {letter ||
              (language === "en"
                ? "Your cover letter will appear here."
                : "A sua carta aparecerá aqui.")}
          </div>
        </div>
      </div>
    </div>
